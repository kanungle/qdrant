...
sidebarTitle: "Agentic RAG With LangGraph"
description: "Traditional Retrieval-Augmented Generation (RAG) systems follow a straightforward path: query → retrieve → generate. Sure, this works well for many scenarios. But let’s face it—this linear approach often struggles when you’re dealing with complex queries that demand multiple steps or pulling together diverse types of information."
---
...
You start by entering a query or request through an interface, like a chatbot or a web form. This query is sent straight to the AI Agent, the brain of the operation.
</Step>

<Step title="AI Agent Processes the Query">
The AI Agent analyzes your query, figuring out what you’re asking and which tools or data sources will best answer your question.
</Step>

<Step title="Tool Selection">
Based on its analysis, the AI Agent picks the right tool for the job. Your data is spread across two vector databases, and depending on the query, it chooses the appropriate one. For queries needing real-time or external web data, the agent taps into a web search tool powered by BraveSearchAPI.
</Step>
...
The results roll in:
<br/>• Vector Database 1 and 2 return the most relevant documents.
<br/>• The Web Search Tool provides up-to-date or external information.
</Step>

<Step title="Response Generation">
Using a text generation model (like GPT), the AI Agent crafts a detailed and accurate response tailored to your query.
</Step>

<Step title="User Response">
The polished response is sent back to you through the interface, ready to use.
</Step>
...
If you don’t already have one, head to [Qdrant Cloud](https://cloud.qdrant.io) and sign up.
</Step>

<Step title="Set Up a Cluster">
Log in to your account and find the **Create New Cluster** button on the dashboard. Follow the prompts to configure:
<br/>• Select your preferred region.
<br/>• Choose the free tier for testing.
</Step>

<Step title="Secure Your Details">
Once your cluster is ready, note these details:
<br/>• **Cluster URL:** e.g., `https://xxx-xxx-xxx.aws.cloud.qdrant.io`
<br/>• **API Key**
</Step>
...
<Frame caption="Agentic RAG with LangGraph">

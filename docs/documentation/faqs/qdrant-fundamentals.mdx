...
sidebarTitle: "Qdrant Fundamentals"
---

## Vectors

<AccordionGroup>
  <Accordion title="What is the maximum vector dimension supported by Qdrant?">
    In dense vectors, Qdrant supports up to 65,535 dimensions.
  </Accordion>

<Accordion title="What is the maximum size of vector metadata that can be stored?">
  There is no inherent limitation on metadata size, but it should be [optimized
  for performance and resource usage](). Users can set upper limits in the
  configuration.
</Accordion>

{" "}

<Accordion title="Can the same similarity search query yield different results on different machines?">
  Yes, due to differences in hardware configurations and parallel processing,
  results may vary slightly.
</Accordion>
<Accordion title="How do I choose the right vector embeddings for my use case?">
  This depends on the nature of your data and the specific application. Consider
  factors like dimensionality, domain-specific models, and the performance
  characteristics of different embeddings.
</Accordion>
<Accordion title="How does Qdrant handle different vector embeddings from various providers in the same collection?">
  Qdrant natively [supports multiple vectors per data point](), allowing different
  embeddings from various providers to coexist within the same collection.
</Accordion>
    ...
    <Accordion title="Why the amount of indexed vectors doesn’t match the amount of vectors in the collection?">
  ...
  Make sure to check that the collection status is green and that the number of unindexed vectors smaller than indexing threshold.
    ...
    <Accordion title="Why collection info shows inaccurate number of points?">
   Collection info API in Qdrant returns an approximate number of points in the collection. If you need an exact number, you can use the [count]() API.
  </Accordion>
    <Accordion title="Vectors in the collection don’t match what I uploaded.">
...
## Search
<AccordionGroup>
  <Accordion title="How does Qdrant handle real-time data updates and search?">
  ...
  <Accordion title="My search results contain vectors with null values. Why?">
 ...
 If you’re still seeing "vector": null in your results, it might be that the vector you’re passing is not in the correct format, or there’s an issue with how you’re calling the upsert method.
  ...
  <Accordion title="How can I search without a vector?">
   ...
   <Accordion title="Does Qdrant support a full-text search or a hybrid search?">
...
<AccordionGroup>
  <Accordion title="How many collections can I create?">
 ...
 Please read more about collections, isolation, and multiple users in our Multitenancy tutorial.
  ...
  <Accordion title="How do I upload a large number of vectors into a Qdrant collection?">
    Read about our recommendations in the bulk upload tutorial.
  </Accordion>

  <Accordion title="Can I only store quantized vectors and discard full precision vectors?">
...
<AccordionGroup>
  <Accordion title="Is Qdrant compatible with CPUs or GPUs for vector computation?">
Qdrant primarily relies on CPU acceleration for scalability and efficiency. However, we also support GPU-accelerated indexing on all major vendors.
  ...
  <Accordion title="Do you guarantee compatibility across versions?">
  ...
  <Accordion title="Do you support downgrades?">
  ...
  <Accordion title="How do I avoid issues when updating to the latest version?">
  ...
  <Accordion title="Should I create payload indexes before or after uploading?">
  ...
  <Accordion title="Should I create one Qdrant collection per user?">
...
<AccordionGroup>
  <Accordion title="Is it possible to scale down a Qdrant Cloud cluster?">
